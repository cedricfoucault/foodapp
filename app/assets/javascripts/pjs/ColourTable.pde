...
public class ColourTable implements Serializable
{
    ...
    public static final float NO_VALUE = FLOAT_MIN_VALUE;
    ...
    static final long serialVersionUID = 790808050;

    private ArrayList<ColourRule> cTable;  // Vector of colour rule objects.
    ...
    public ColourTable()
    {
        cTable = new ArrayList<ColourRule>();
        cTableType = COLOUR_RULES;      // Uses colour rules by default.
        name = "User";
        isDiscrete = false;
        
        cTable.add(new ColourRule(-FLOAT_MAX_VALUE,255,255,255,0)); // Default background is transparent.
    }       
    ...
    public ColourTable(ColourTable oldColourTable)
    {
        ...
        cTable = new ArrayList<ColourRule>();
        cTable.add(new ColourRule(-FLOAT_MAX_VALUE,255,255,255,0));
   ...
   public boolean removeColourRule(float index)
   {
    ...
    public float getMaxIndex()
    {
        ...
        ColourRule rule = cTable.get(cTable.size() - 1);
  ...
  public ListModel toListModel()
    ...
    /** Returns the rules associated with the colour table
      ...
      */
    public ArrayList<ColourRule> getColourRules()
    {
        return cTable;
    }
     ...
     */  
    public String getHexString(int colour)
    {
     ...
     */  
    public String getHexStringABGR(int colour)
    {
        int alf   = (colour & ALF) >> 24;
        // Convert to unsigned integer.
        if (alf < 0) 
     ...
     */
    public ColourTable getPresetColourTable(int type)
    {
        return getPresetColourTable(type,NaN,NaN);
    }
     ...
     */
    private float getDefaultMin(int type)
    {
        switch (type)
        {
     ...
     */
    private float getDefaultMax(int type, float min)
    {
      float validMin = min;
     ...
     * @param min Minimum value. If <code>NaN</code>, the scheme's default minimum is used.
     * @param max Maximum value (ignored for ColorBrewer categorical schemes). If <code>NaN</code>, the scheme's default
     *            maximum is used.
     * @return Colour table scaled between given <code>min</code> and <code>max</code> values.
     */
    public ColourTable getPresetColourTable(int type, float min, float max)
    {
      float validMin = min;
      float validMax = max;
    ...
    /** Creates a colour table from the rules defined in the file with the given name. The file 
      ...
      */
    public ColourTable readFile(String fileName)
    {       
        // Extract colour rules from XML file.   
        DOMProcessor dom = new DOMProcessor(fileName);
        if (dom.isEmpty())
        {
            println("Could not extract XML colourtable data from '"+fileName+"'.");
            return null;
        }
    ...
    /** Creates a colour table from the rules defined in the given input stream. The file should
      ...
      * @return Colour table created or null if problems creating table from file.
      */
    public ColourTable readFile(InputStream inStream)
    {
        // Extract colour rules from XML file.   
        DOMProcessor dom = new DOMProcessor(inStream);
        if (dom.isEmpty())
        {
            println("Could not extract XML colourtable data from input stream.");
            return null;
        }
        return processDom(dom);
    }
    ...
    /** Writes the given colour table out as a file using the given name. The file can
      ...
      */
    public boolean writeFile(ColourTable colourTable, String fileName)
    {
        if (colourTable == null)
        {
            println("No colour table provided to write to file.");
            return false;
        }
        ...
        {
            ArrayList<ColourRule> rules = colourTable.getColourRules();
            ColourRule colRule;
        
            for (int i=1; i<rules.size(); i++)  // Excludes first value which is transparent.
            {
                colRule = rules.get(i);
                org.w3c.dom.Node rule = dom.addElement("rule", colRule.toString(colRule.getlColour()),root);
    ...
    /** Writes the given colour table out as a file using the given output stream. The file can
      ...
      * @return True if file written successfully.
      */
    public boolean writeFile(ColourTable colourTable, OutputStream outStream)
    {
        if (colourTable == null)
        {
            println("No colour table provided to write to file.");
    ...
    /** Creates the colour table from the given DOM.
      ...
      * @return New colour table.
      */
    private ColourTable processDom(DOMProcessor dom)
    {
        ...
        if (nodes.length <1)
        {
            println("No colour rules found in colourtable file.");
            return null;
        }
